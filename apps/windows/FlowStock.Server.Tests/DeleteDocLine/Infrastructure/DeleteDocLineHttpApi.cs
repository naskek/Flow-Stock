...
internal static class DeleteDocLineHttpApi
{
    ...
    public static async Task<DeleteDocLineEnvelope> DeleteAsync(HttpClient client, string docUid, DeleteDocLineRequest request)
    {
        using var response = await client.PostAsJsonAsync($"/api/docs/{docUid}/lines/delete", request);
        response.EnsureSuccessStatusCode();
        var payload = await response.Content.ReadFromJsonAsync<DeleteDocLineEnvelope>(JsonOptions);
...
internal sealed class DeleteDocLineEnvelope
{
    [JsonPropertyName("ok")]
    ...
    public DeleteDocLinePayload? Line { get; init; }
}

internal sealed class DeleteDocLinePayload
{
    [JsonPropertyName("id")]
